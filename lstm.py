...
def build(P,name,input_size,hidden_size):
	...
	P[name_init_hidden] = 0.5 * np.random.randn(hidden_size)
	P[name_init_cell]   = 0.5 * np.random.randn(hidden_size)
	#(0.1 * 2) * (np.random.rand(hidden_size) - 0.5)
	#(0.1 * 2) * (np.random.rand(hidden_size) - 0.5)
...
def build_step(P,name,input_size,hidden_size):
	...
	name_b        = "b_%s"%name
	P[name_W_input]  = 0.5 * np.random.randn(input_size,  hidden_size * 4)
	P[name_W_hidden] = 0.5 * np.random.randn(hidden_size, hidden_size * 4)
	P[name_W_cell]   = 0.5 * np.random.randn(hidden_size, hidden_size * 3)
...
#	(0.1 * 2) * (np.random.rand(hidden_size,hidden_size * 3) - 0.5)
	...
	bias_init[1*hidden_size:2*hidden_size] = 2.5
	P[name_b] = bias_init
	biases = P[name_b]
